import React from "react";
import Dialog from "../features/credits/Dialog";
import Plan from "../features/credits/Plan";
import Warning from "../components/Warning";
import { useState, useEffect } from "react";
import { FaLock } from "react-icons/fa6";
import { PulseLoader } from "react-spinners";
import { IoMdCloseCircle } from "react-icons/io";
import { MdOutlineRefresh } from "react-icons/md";
import { LuMail } from "react-icons/lu";
import { GoDotFill } from "react-icons/go";
import { twMerge } from "tailwind-merge";
import CustomError from "../components/CustomError";
import { use } from "react";
import { AuthContext } from "../utils/AuthProvider";
import { sleep } from "../utils/sleep"
import { customFetch } from "../utils/customFetch";

//static variable
const retrySupportButton =
  "flex items-center gap-1.5 px-4 py-1.5 rounded-lg text-[0.9rem] border duration-200 ease-in hover:-translate-y-0.5 hover:cursor-pointer max-lg:py-2";

const BuyCredits = () => {

  //givinhg credits of the page
  document.title = "Top Up - ThinkTweet"
  
  //using state to show the loader
  const [showLoader, setShowLoader] = useState(false);
  const [err, setErr] = useState("");

  //fn to create a plan name to be send to the server
  const createPlanName = (IdOfPlan) =>{
    if (IdOfPlan === "4g78qs") {
      return "premium"
    } else if (IdOfPlan === "5g56qs") {
      return "pro";
    } else if (IdOfPlan === "6g564d8qs") {
      return "pro-max";
    } else {
      return "unknown"
    }
  }

  //function to talk to the server
  const talkServerForStripe = async (planId) => {
    // console.log("Inside talkServerForStripe function...")
    // console.log("Showing the loader")
    setShowLoader(true);

    //getting a plan name
    // console.log("Creating a plan name...")
    const planName = createPlanName(planId)

    //checking whether the expected plan is there or not
    // console.log("Checking whether expected plan name exist or not")
    if (planName === "unknown") {
      // console.log("Unexpected plan name found")
      setShowLoader(false)
      setErr(
        "Unexpected plan id found. Please try again. If the issue persists, contact our support team.",
      );
      return
    }
    // console.log("Validation for plan name passed")

    //creating a body object
    // console.log("Creating a body object for the request")
    const bodyObj = {
      plan : planName
    }

    //establishing a connection with a backend
    // console.log("Beginnning the server talk")
    const serverObject = await customFetch("payment", bodyObj)
    // console.log("Server Object: ", serverObject)

    //sleeping for few seconds
    await sleep(3000)

    if (serverObject.statusCode === 401) {
      // console.log("Seems like action access token is expired")

      //talking to the refresh router
      // console.log("Talking with server to generate refresh token")
      const refreshObject = await customFetch("refresh")

      if (refreshObject.success) {
        // console.log("Access token generated")

        //again talking to the payment router
        // console.log("Talking to the server again to generate checkout session")
        const finalObject = await customFetch("payment", bodyObj)
        // console.log("Final Object: ", finalObject)

        if (finalObject.success) {
          // console.log("Checkout session generated")
          window.location.href = finalObject.message.sessionUrl;
        } else {
          setErr(finalObject.message)
        }
      } else {
        setErr(refreshObject.message)
      }
    } else if (serverObject.success) {
      // console.log("Session checkout generated by stripe")
      window.location.href = serverObject.message.sessionUrl;
    } else {
      setErr(serverObject.message)
    }

    //turning off the loader
    setShowLoader(false)
  };

  //forcing the user to move above page so that they can see the error
  useEffect(() => {
    if (err) {
      window.scrollTo({ top: 0, left: 0, behavior: "instant" });
    }
  }, [err]);

  //checking whehter the verified user is logged in or not
  const auth = use(AuthContext);

  return auth.isAuthenticated ? (
    <div className={twMerge("bg-[#f8fafc]", err && "pt-15")}>
      {/* <Warning /> */}
      {showLoader && (
        <div className="fixed z-100 inset-0 bg-[#f8fafc]/60 backdrop-blur-xs pointer-events-auto">
          <div className="w-max ml-auto mt-12 mr-45 shadow-2xl p-2.5 px-4 rounded-lg flex items-center gap-3 bg-white max-lg:mr-30 max-md:mr-10 max-phone:mr-0 max-phone:ml-0 max-phone:justify-self-center max-phone:px-3">
            <FaLock
              className="bg-[#eef2ff] text-brand-tertionary p-2 rounded-lg"
              size={30}
            />
            <div>
              <h5 className="font-semibold text-base -mb-1 text-brand-primary">
                Redirecting to Stripe
              </h5>
              <p className="text-sm text-brand-secondary">
                Establishing secure connection
              </p>
            </div>
            <PulseLoader className="ml-5" color="#4f46e5" size={8} />
          </div>
        </div>
      )}
      {err && (
        <div className="flex border rounded-lg bg-[#fef2f2] border-[#fecaca] shadow-xl items-center justify-center justify-self-center gap-4 py-4 px-8 w-max max-lg:px-4 max-md:flex-col max-md:items-start max-md:w-[90%] max-md:p-6">
          <IoMdCloseCircle
            className="p-3 size-13 border-[#fecaca] bg-[#fee2e2] animate-pulse rounded-full border max-md:mb-2"
            size={40}
            color="#ef4444"
          />
          <div>
            <div className="flex items-center mb-1 max-md:flex-col max-md:items-start max-md:mb-4">
              <h5 className="text-base font-semibold mr-3 text-[#b91c1c] max-md:mb-1.5">
                Transaction Failed
              </h5>
              <p className="flex items-center gap-1 bg-white uppercase text-[#f87171] border text-xs rounded-2xl py-0.5 px-3">
                <GoDotFill color="#ef4444" />
                Err_occured
              </p>
            </div>
            <p className="text-sm w-lg tracking-wide text-brand-secondary max-lg:w-md max-md:w-full">
              {err}
            </p>
          </div>
          <div className="flex gap-2.5 ml-4 max-lg:flex-col max-md:ml-0 max-md:w-full max-md:mt-2">
            <button
              onClick={() => setErr("")}
              className={twMerge(
                retrySupportButton,
                "bg-[#ef4444] text-white hover:bg-[#b91c1c] max-lg:justify-center",
              )}
            >
              <MdOutlineRefresh />
              Try Again
            </button>
            <a
              href=""
              className={twMerge(
                retrySupportButton,
                "bg-white border-brand-fourth text-brand-secondary hover:text-[#ef4444] hover:border-[#ef4444] max-lg:justify-center",
              )}
              onClick = {(e) => e.preventDefault()}
            >
              <LuMail />
              Contact Support
            </a>
          </div>
        </div>
      )}

      {/* Components will mounts... */}
      <Dialog />
      <Plan funcToTalkServer={talkServerForStripe} />
    </div>
  ) : (
    <CustomError content="401 Unauthorized" />
  );
};

export default BuyCredits;
